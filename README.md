...
The entire package is pip installable. Assuming you have all listed requirements, a simple `pip install .` suffices to get the package up and running, in addition to installing from `requirements.txt`. However, an installation script `./setup.sh` can also be run after downloading this repository.
